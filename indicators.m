...
%%技术指标，应用格式： 输出=indicate（价格矩阵，kdj，其他参数）；  kdj376行，macd638行
...
%==========================================================================
    ...
    case 'kdj'     % KDJ Indicator
        ...
        % 2020-09-04 修正为空bug
        % fpctk 存在空值时，fpctk(~isnan(fpctk)) 与 observ 长度不一致将导致 ema 计算索引超界限
        % Fast %D
        fpctd                = nan(size(cl));
        fpctd(~isnan(fpctk)) = ema(fpctk(~isnan(fpctk)),dperiods,sum(~isnan(fpctk)));
